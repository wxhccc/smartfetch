...
function returnRequestLink(url, data, baseCfg) {
    const paramsStr = '?' + queryString.stringify(data);
...
function appendDataToForm(formdata, data) {
    ...
    for (let i in data) {
        ...
        if (typeof item === 'number' || typeof item === 'string') {
            ...
            const stringifyData = queryString.stringify(item);
...
function SFRequest (config) {
    ...
    function createRequestConfig(url, data = {}, method = 'GET', returnLink = false, enctype = 'json') {
        ...
        // query methods
        if (canUseLink) {
            useFetch ? (result.url += `?${queryString.stringify(handleData)}`) : (result.params = data);
            return result;
        }
        ...
        if (useFetch) {
            ...
            result.body = isFormData ? data : (enctype === 'json' ? JSON.stringify(data) : queryString.stringify(data));
        }
        else {
            result.data = (isFormData || enctype === 'json') ? data : queryString.stringify(data);
        }
        return result;
    }
